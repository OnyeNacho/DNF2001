...
UBOOL UWindowsViewport::diSetupKeyboardMouse()
{
	...
	// Workaround for unplayable mouse behavior when played through wine
	// TODO: Remove this workaround when we do a proper fix/rewrite

	// Detect wine
	HMODULE ntdll = LoadLibraryW(L"ntdll.dll");
	bool wine = GetProcAddress(ntdll, "wine_get_version") != NULL;
	FreeLibrary(ntdll);
	// The default coop flags on windows
	DWORD mouseCoopFlags = DISCL_FOREGROUND | DISCL_EXCLUSIVE;
	if (wine)
	{
		debugf(_T("Wine detected. Applying workaround for mouse input."));
		mouseCoopFlags = DISCL_FOREGROUND | DISCL_NONEXCLUSIVE;
	}
	// End of wine mouse workaround

	hr = diMouse->SetCooperativeLevel( Window->hWnd,  mouseCoopFlags);
	if ( FAILED( hr ) )
	{
