...
final function vector GetAspectOffset(vector baseRes, float X, float Y, bool addOffset)
{
	...
	XRatio = ClipX / baseRes.X;
	YRatio = ClipY / baseRes.Y;
	bestRatio = FMin(XRatio, YRatio);

	if(addOffset)
	{
		XDiff = (ClipX - (baseRes.X * bestRatio)) / 2.0;
		YDiff = (ClipY - (baseRes.Y * bestRatio)) / 2.0;
	}
