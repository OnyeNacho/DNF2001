...
simulated event TickVibration( float DeltaTime )
{
	...
	if (((VibrationTarget.X != 0.0) || (VibrationTarget.Z != 0.0)) && (Level.TimeSeconds < VibrationStart + VibrationDuration))
	{
		...
		VibrationTarget.X = Sin(Level.TimeSeconds * 75.f) * (VibrationIntensity/4) * scale;
		VibrationTarget.Z = Cos(Level.TimeSeconds * 50.f) * (VibrationIntensity/4) * scale;
...
event PlayerCalcView(out actor ViewActor, out vector CameraLocation, out rotator CameraRotation )
{
	...
	else
	{
		...
		// THIS IS NOT THE RIGHT PLACE TO BE DOING THIS BTW 
		// IT SHOULD BE CalcDrawOffset BUT I SOMEHOW MANAGED 
		// TO GET IT HERE AND IT BROKE THERE SO ?? -x
		if ((65535 - ViewRotation.Pitch) < 32768)
			CameraLocation	+= (Y+Z-X) * 0.5 * (Float(65535 - ViewRotation.Pitch) / 16384.0);
		
		// if someone decides to do #174:
